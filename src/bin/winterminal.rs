...
use serde::Serialize;
use winterminal::integration::{IntegrationConfig, doctor, install, plan, uninstall};
use winterminal::platform::windows::relaunch_current_process_elevated;
use winterminal::{
...
fn execute(cli: Cli) -> AppResult<ExitCode> {
    ...
    match cli.command.unwrap_or(CliCommand::Launch) {
        CliCommand::Config { path_only, edit } => configure(path_only, edit),
        CliCommand::Plan => {
